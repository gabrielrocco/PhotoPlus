...
class MultiBandHSV: CIFilter
{
    let multiBandHSVKernel: CIColorKernel =
    {
      ...
      //  shaderString += " if (lum > 0.88) {lum = 0.88;}"
        
        //Essas 3 linhas que eu adicionei depois de aprender arduamente o que este código bizarro faz resolvem o problema de áreas com poucas informações que estragavam a imagem ao serem alteradas na saturação ou na luminância
        //Basicamente ele limita a interação/edição de pixels com baixa saturação e com alta luminancia pois estes pixels não tem informação suficiente para trazer em resultado agradavel
        
        //se a saturação for menor que 0.6, nao se aplica filtro de luminancia
       shaderString += " if (hsv.y < 0.6) {lum = hsv.z;}"
        //se a saturação for menor que 0.38 não se aplica filtro de saturação
         shaderString += " if (hsv.y < 0.38) {sat = hsv.y;}"
          shaderString += " if (hsv.y < 0.35 && lum > 0.6) {sat = hsv.y;}"
      shaderString += " if (lum > 0.88 ) {sat = hsv.y;}"
