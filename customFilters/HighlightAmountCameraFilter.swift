...
class highlightAmountFilter: CIFilter
{
    let highlightAmountFilterKernel: CIColorKernel =
    {
        ...
        //highli
        shaderString += " if (hsv.z >= 0.97 && hsv.y < 0.1){hue = 0; "
        shaderString += "sat = 1;"
        shaderString += "lum = 0.5;}"
        ...
        shaderString += "return vec4(hsv2rgb(hsv), 1);"
        shaderString += "}"
    ...
    override var outputImage: CIImage?
    {
        ...
        return highlightAmountFilterKernel.apply(extent: inputImage.extent,
